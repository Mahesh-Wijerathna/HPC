...
void generate_combinations(char *prefix, int depth, int max_length, const char *target_hash, int *found_flag, int rank, int size, int start_idx) {
    ...
    for (int i = (depth == 0 ? start_idx : 0); i < charset_size && !*found_flag; i += (depth == 0 ? size : 1)) {
        prefix[depth] = charset[i];
        prefix[depth + 1] = '\0';
        generate_combinations(prefix, depth + 1, max_length, target_hash, found_flag, rank, size, start_idx);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    for (int len = 1; len <= max_length; len++) {
        ...
        generate_combinations(buffer, 0, len, target_hash, &found, rank, size, rank);
