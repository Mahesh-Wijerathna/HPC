...
int main() {
    ...
    for (int len = 1; len <= max_length && !found; len++) {
        ...
        unsigned long temp;
        #pragma omp parallel for shared(found, target_hash, charset, charset_size, len, total) private(temp) default(none) schedule(dynamic)
        for (unsigned long n = 0; n < total; n++) {
            if (found) continue; // early exit if another thread found the result

            char candidate[len + 1]; // stack-allocated per thread
